...
class UsersController < ApplicationController
  ...
  before_filter :journal_data, :only => [:index, :new, :edit, :show, :users_rc]
    ...
    @users = User.joins(:core_project_scores).where("core_project_scores.project_id = ? AND core_project_scores.score > 0", @project.id).where(:type_user => [4,7]).order("core_project_scores.score DESC").paginate(:page =>params[:page])
  end

  def show_top
    @project = Core::Project.find(params[:project])
    #@users = User.scope_score_name(params[:score_name]).where('admin=?', false).paginate(:page =>params[:page])
    @users = User.joins(:core_project_scores).where("core_project_scores.project_id = ? AND core_project_scores.score > 0", @project.id).where(:type_user => [4,7]).order("core_project_scores.#{params[:score_name]} DESC").paginate(:page =>params[:page])
