class Novation::PostsController < PostsController
  ...
  def index
    ...
    respond_to do |format|
      ...
      format.json { render json: @posts.each_with_index.map { |item, index| {id: item.id, index: index+1, title: item.title.present? ? item.title : 'Пакет без названия', content: trim_content(item.project_change, 200), approve_status: item.approve_status, useful: item.useful, admin_panel: (current_user.boss? or current_user.role_expert? or current_user.stat_expert?),
