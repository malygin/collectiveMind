class Novation::PostsController < PostsController
  ...
  def index
    ...
    respond_to do |format|
      ...
      format.json do render json: @posts.each_with_index.map { |item, index| {id: item.id, index: index+1, title: item.title.present? ? item.title : 'Пакет без названия', content: trim_content(item.project_change, 200), approve_status: item.approve_status, useful: item.useful, admin_panel: boss?,
                                                                              ...
                                                                              comments: item.comments.preview.map { |comment| {id: comment.id, date: Russian::strftime(comment.created_at, '%k:%M %d.%m.%y'), user: comment.user.to_s, content: trim_content(comment.content, 50)} }} } end
