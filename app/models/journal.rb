class Journal < ActiveRecord::Base
  ...
  # scope :closed_proc, -> events_ignore { where.not(journals: {type_event: events_ignore})}
  # where("core_projects.type_access IN (#{list_type.join(", ")}) OR core_projects.id IN (#{closed_projects.join(", ")})")
  ...
  # scope :by_period, -> date_begin, date_end, date_all { where("DATE(journals.created_at + time '04:00') BETWEEN ? AND ?",
  # ((date_begin.present? and date_all.nil?) ? date_begin : '1900-01-01'), ((date_end.present? and date_all.nil?) ? date_end : '2100-01-01')) }
