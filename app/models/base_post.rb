module BasePost  extend ActiveSupport::Concern
  included do
    ...
    has_many :admins_pro, :through => :post_votings_pro, :source => :user,:conditions => [:type_user => [1,6]]
    has_many :admins_vote, :through => :post_votings, :source => :user,:conditions => [:type_user => [1,6]]

    has_many :admins_against, :through => :post_votings_against, :source => :user ,:conditions => [:type_user => [1,6]]
