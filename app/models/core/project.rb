class Core::Project < ActiveRecord::Base
  ...
  has_many :discontent_accepted_post, -> { where status: 0 }, class_name: 'Discontent::Post'
  ...
  def discontents_without_aspect
    discontents.includes(:discontent_post_aspects).where(discontent_post_aspects: { post_id: nil })
  end

  def get_free_votes_for(user, stage)
    case stage
      when :collect_info
        proc_main_aspects.size - user.voted_aspects.by_project(id).size
      when :discontent
        discontent_for_vote.size - user.voted_discontent_posts.by_project(id).size
      when :concept
        concept_ongoing_post.size - user.voted_concept_post.by_project(id).size
      when :novation
  ...
  def get_other_aspects_sorted_by(sort_rule)
    if sort_rule == 'sort_by_comments'
      other_main_aspects.sort_comments
    elsif sort_rule == 'sort_by_date'
      other_main_aspects.created_order
    else
      other_main_aspects
    end
  end
  ...
  def get_main_aspects_sorted_by(sort_rule)
    if sort_rule == 'sort_by_comments'
      proc_main_aspects.sort_comments
    elsif sort_rule == 'sort_by_date'
      proc_main_aspects.created_order
    else
      proc_main_aspects
    end
  end
